...
def dataset_check(output_dir):
    try:
        all = glob.glob(os.path.join(output_dir, '*'))
        print('모든 데이터셋 이미지 개수:', len(all))
...
def customized_dataset_check(dataset):
    ...
    try:
        ...
        if shape != torch.Size([3, 120, 120]):
            print(
                f'{dataset.mode}의 __getitem__ 함수가 반환하는 img의 크기가 올바르지 않습니다. transform 과정에서 이미지 크기를 120으로 고정하는 부분을 실수로 수정했는지 확인하시기 바랍니다.')
            item_flag = False

    except AttributeError:
        print(
            f'{dataset.mode}의 __getitem__ 함수가 제대로된 img를 반환하지 않거나 transform이 img에 적용되지 않았습니다. __getitem__함수에서 transform을 적용하도록 구현했는지 확인하시기 바랍니다.')
        item_flag = False

    try:
        if len(dataset) != 800:
            print(
                f'{dataset.mode}의 __len__ 함수가 train_data의 데이터 갯수를 500이 아닌 {len(dataset)}으로 반환하고 있습니다. 가지고 있는 데이터셋 또는 __len__함수 구현에 문제가 없는지 다시 확인하시기 바랍니다.'.format(
                    len(dataset)))
            len_flag = False
        ...
        if item_flag and len_flag:
            print('Food Classifier를 잘 구현하셨습니다! 이어서 진행하셔도 좋습니다.')
    except Exception as e:
        print('체크 함수를 실행하는 도중에 다음과 같은 문제가 발생했습니다. 코드 구현을 완료했는지 다시 검토하시기 바랍니다.')
